...
def get_browsefeatured_playlists(*args)
  header_params = {}
  ...
  uri_string = "#{arg_hash['endpoint_api']}/browse/featured-playlists" % path_params
...
def format_params(key_values)
 ...
 key_values.each { | key, value |
  ...
  op_params = [
      ...
      op_param('playlists', 'body', 'playlists', 'playlists'),
      op_param('timestamp', 'query', 'timestamp', 'timestamp'),
    ]
  op_params.each do |i|
    location = i[:location]
    name     = i[:name]
...
def task
  ...
  params = STDIN.read
  result = get_browsefeatured_playlists(params)
